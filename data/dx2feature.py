...
def findvalueall(dotcloud,gridsize,origin,delta,value):
    ind3d=np.floor((dotcloud-origin)/delta)
#     print(np.max(ind3d))
#     print(gridsize)
    ind1d=ind3d[:,2]+ind3d[:,1]*gridsize[2]+ind3d[:,0]*gridsize[1]*gridsize[2]
    ind1dx0=ind3d[:,2]+ind3d[:,1]*gridsize[2]+(ind3d[:,0]-1)*gridsize[1]*gridsize[2]
    ind1dx1=ind3d[:,2]+ind3d[:,1]*gridsize[2]+(ind3d[:,0]+1)*gridsize[1]*gridsize[2]
    ind1dy0=ind3d[:,2]+(ind3d[:,1]-1)*gridsize[2]+ind3d[:,0]*gridsize[1]*gridsize[2]
    ind1dy1=ind3d[:,2]+(ind3d[:,1]+1)*gridsize[2]+ind3d[:,0]*gridsize[1]*gridsize[2]
    ind1dz0=ind3d[:,2]-1+ind3d[:,1]*gridsize[2]+ind3d[:,0]*gridsize[1]*gridsize[2]
    ind1dz1=ind3d[:,2]+1+ind3d[:,1]*gridsize[2]+ind3d[:,0]*gridsize[1]*gridsize[2]
    
    
    gx=value[ind1dx1.astype(int)]-value[ind1dx0.astype(int)]
    gy=value[ind1dy1.astype(int)]-value[ind1dy0.astype(int)]
    gz=value[ind1dz1.astype(int)]-value[ind1dz0.astype(int)]
#     temp=value.reshape(gridsize)
#     print temp[ind3d[:,0].astype(int),ind3d[:,1].astype(int),ind3d[:,2].astype(int)]

    return np.vstack((value[ind1d.astype(int)],gx,gy,gz))

def findvalueallm(dotcloud,gridsize,origin,delta,value):
    ind3d=np.floor((dotcloud-origin)/delta).astype(int)
#     print(np.max(ind3d,axis=0))
#     print(gridsize)

    value=value.reshape(gridsize[0],gridsize[1],gridsize[2])
#     print(value.shape)
#     print(ind3d.shape)
    gx,gy,gz=np.gradient(value)

    return np.vstack((value[ind3d[:,0],ind3d[:,1],ind3d[:,2]],gx[ind3d[:,0],ind3d[:,1],ind3d[:,2]],gy[ind3d[:,0],ind3d[:,1],ind3d[:,2]],gz[ind3d[:,0],ind3d[:,1],ind3d[:,2]]))
