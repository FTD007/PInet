function [vertex, face, color,vector] = read_wrl2(filename)
...
while ( tempstr ~= -1)
    tempstr = fgets(fp);   % -1 if eof
%     if( ~isempty(findstr(tempstr,key)) )
    if( ~isempty(strfind(tempstr,key)) )
        [color,nc] = fscanf(fp,'%f%c %f%c %f%c', Inf);
        
        color = reshape(color, [6,length(color)/6]);
        color([2 4 6],:) = [];
        ...
        while nc>0
            ...
            if nc>0
                ...
                color = [color, reshape(cvals, 3, nc/3)];
            end
        end
        end
...
%     if( ~isempty(findstr(tempstr,key)) )
    if( ~isempty(strfind(tempstr,key)) )
        [vector,nc] = fscanf(fp,'%f%c %f%c %f%c', Inf);
        
        vector = reshape(vector, [6,length(vector)/6]);
        vector([2 4 6],:) = [];
        ...
        while nc>0
            ...
            if nc>0
                ...
                vector = [vector, reshape(cvals, 3, nc/3)];
            end
        end
        end
